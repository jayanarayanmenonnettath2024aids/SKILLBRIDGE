...
const ProfileSetup = () => {
  ...
  const handleSkillToggle = (skill) => {
    ...
    if (skills.includes(skill)) {
      setFormData({ ...formData, skillsSelected: skills.filter(s => s !== skill) });
    } else {
      setFormData({ ...formData, skillsSelected: [...skills, skill] });
    }
  };
          ...
          <div className="form-section">
            ...
            <div className="domain-grid">
              {domainOptions.map((domain) => (
                <button
                  key={domain.value}
                  className={`domain-card ${formData.interestedDomain === domain.value ? 'selected' : ''}`}
          ...
          <div className="form-section">
            ...
            <div className="skills-grid">
              {skillsOptions.map((skill) => (
                <button
                  key={skill}
                  className={`skill-tag ${formData.skillsSelected.includes(skill) ? 'selected' : ''}`}
                  onClick={() => handleSkillToggle(skill)}
                  type="button"
                >
                  {skill}
                  {formData.skillsSelected.includes(skill) && <CheckCircle size={14} />}
                </button>
              ))}
            </div>
          </div>
