...
It fetches user skills and required skills from MongoDB and performs
...
class SkillGapService:
    ...
    def calculate_skill_gap(user_id: str) -> Dict[str, Any]:
        ...
        required_data = db.requiredSkills.find_one({'role': user_role})
        if not required_data:
            raise ValueError(f"Required skills for role {user_role} not found")
        
        required_skills = required_data.get('skills', {})
    ...
    def get_all_users() -> List[Dict[str, Any]]:
        ...
        """
        db = get_db()
        users_cursor = db.users.find({}, {'_id': 1, 'name': 1, 'role': 1})
    ...
    def get_required_skills_by_role(role: str) -> Dict[str, Any]:
        ...
        """
        db = get_db()
        data = db.requiredSkills.find_one({'role': role})
