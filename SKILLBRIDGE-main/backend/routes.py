...
from services.firebase_service import get_db, get_firebase_app
...
def register_user():
    ...
    try:
        ...
        aadhaar_query = users_ref.where('aadhaar', '==', aadhaar).limit(1).stream()
        for doc in aadhaar_query:
...
def update_profile():
    ...
    db.collection('profiles').document('current_profile').set(data)
...
def get_profile():
    db = get_db()
    doc = db.collection('profiles').document('current_profile').get()
...
def check_eligibility():
    ...
    # Use stored profile from Firestore if not provided in request
...
def upload_resume():
    """
    Upload resume PDF file and store in Firebase Storage
    ...
    try:
        ...
        try:
            # Store file locally (fallback if Firebase Storage not enabled)
            # Create permanent storage directory
            storage_dir = os.path.join('uploads', 'resumes', user_id)
...
def analyze_resume():
    ...
    try:
        ...
        try:
            ...
            # Store analysis in Firestore (even if it's fallback analysis)
            print("💾 Storing analysis in Firestore...")
            db = get_db()
            db.collection('users').document(user_id).set({
...
def search_jobs():
    ...
    try:
        ...
        # Store job matches in Firestore for caching
        db.collection('users').document(user_id).set({
...
def update_user_profile():
    ...
    try:
        ...
        # Update Firestore
        db = get_db()
        db.collection('users').document(user_id).update(profile_updates)
...
def seed_skill_gap_data():
    """
    Seed synthetic data into Firestore for demonstration.
    ...
    try:
        ...
        return jsonify({
            ...
            "info": "Users and required skills have been added to Firestore"
        })
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def get_learning_videos(user_id):
    ...
    try:
        ...
        # Fetch user's resume analysis from Firestore
        analysis_ref = db.collection('resume_analysis').document(user_id)
